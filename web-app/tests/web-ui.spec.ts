...
async function createUploadableDatabaseFile(): Promise<{
  name: string;
  mimeType: string;
  buffer: Buffer;
}> {
  ...
  db.close();
  return {
    name: "manual-import.db",
    mimeType: "application/x-sqlite3",
    buffer: Buffer.from(exported),
  };
}

async function createKeyboardDefaultsDatabaseFile(): Promise<{
  name: string;
  mimeType: string;
  buffer: Buffer;
}> {
...
async function createLongRegisterDatabaseFile(): Promise<{
  ...
  db.close();
  return {
    name: "long-register.db",
    mimeType: "application/x-sqlite3",
    buffer: Buffer.from(exported),
  };
}

async function createWindowedOlderDateDatabaseFile(): Promise<{
  name: string;
  mimeType: string;
  buffer: Buffer;
}> {
  ...
  db.close();
  return {
    name: "windowed-older-date.db",
    mimeType: "application/x-sqlite3",
    buffer: Buffer.from(exported),
  };
}

async function createMassiveHistoryDatabaseFile(): Promise<{
  name: string;
  mimeType: string;
  buffer: Buffer;
}> {
